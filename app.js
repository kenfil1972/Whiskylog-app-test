...
const T = {
  no: {
    brand:'PREMIUM BRENNEVINSJOURNAL', title:"Kenneth's WhiskyLog", version:'WhiskyLog v2.33',
  ...
  en: {
    brand:'PREMIUM SPIRITS JOURNAL', title:"Kenneth's WhiskyLog", version:'WhiskyLog v2.33',
...
/* ===== v2.33 direct replace: ratings, score, tasting actions ===== */
(function(){
  function n(v){ const x=Number(String(v ?? '').replace(',','.')); return Number.isFinite(x)?Math.max(0,Math.min(100,x)):0; }
  window.wlTastingPct = function(t){
    ...
    const legacy=Number(t.average||t.score||0);
    if(Number.isFinite(legacy)&&legacy>0){ if(legacy<=1)return Math.round(legacy*1000)/10; if(legacy<=10)return Math.round(legacy*100)/10; return Math.round(legacy*10)/10; }
    return 0;
  };
  window.tastingPercent = window.wlTastingPct;
  window.tastingTotal = window.wlTastingPct;
  window.fmtPct = function(v){ const x=Number(v); return Number.isFinite(x)&&x>0 ? x.toFixed(1)+' %' : '0.0 %'; };
  window.averageScoreForLibrary = function(libraryId){
    const bottleIds=(state.bottles||[]).filter(b=>b.libraryId===libraryId).map(b=>b.id);
    const vals=(state.tastings||[]).filter(t=>bottleIds.includes(t.bottleId)).map(window.wlTastingPct).filter(x=>x>0);
    return vals.length?Math.round((vals.reduce((a,b)=>a+b,0)/vals.length)*10)/10:'';
  };
  window.wlBottleAvg = function(bottleId){
    const vals=(state.tastings||[]).filter(t=>t.bottleId===bottleId).map(window.wlTastingPct).filter(x=>x>0);
    return vals.length?Math.round((vals.reduce((a,b)=>a+b,0)/vals.length)*10)/10:0;
  };
  window.wlBottleStats = function(bottleId){
    const vals=(state.tastings||[]).filter(t=>t.bottleId===bottleId).map(window.wlTastingPct).filter(x=>x>0);
    if(!vals.length) return {min:0,avg:0,max:0,count:0};
    return {min:Math.round(Math.min(...vals)*10)/10, avg:Math.round((vals.reduce((a,b)=>a+b,0)/vals.length)*10)/10, max:Math.round(Math.max(...vals)*10)/10, count:vals.length};
  };
  function dpl(t){ if(t.mode!=='water')return ''; const d=Number(String(t.drops||'').replace(',','.')); const ml=Number(String(t.ml||'').replace(',','.')); return Number.isFinite(d)&&Number.isFinite(ml)&&d>0&&ml>0 ? Math.round((d/ml)*1000)+' dråper/L' : ''; }
  function row(label,score,note){ return `<div class="scoreLineOne"><span class="scoreName">${label}</span><span class="scorePoints">${esc(score||'-')} poeng</span><span class="scoreNote">${esc(note||'')}</span></div>`; }
  window.tastingDetailItem = function(t){ const water=dpl(t); return `<div class="item tastingDetailItem"><div><div class="title">${esc(t.date||'')} · ${t.mode==='water'?'Med vann':'Neat'} ${water?'· '+water:''}</div><div class="meta">Totalvurdering: <b>${fmtPct(wlTastingPct(t))}</b> · ${esc(t.ml||'')} ml</div><div class="scoreGrid">${row('Visuelt',t.visualScore,t.visualNote)}${row('Lukt',t.aromaScore,t.aromaNote)}${row('Smak',t.tasteScore,t.tasteNote)}${row('Ettersmak',t.finishScore,t.finishNote)}${row('Helhetsinntrykk',t.overallScore,t.overallNote)}</div>${t.notes?`<p class="small">${esc(t.notes)}</p>`:''}</div><div class="actions tastingActions"><button class="ghost" type="button" data-wl-action="edit-tasting" data-id="${t.id}">Rediger</button><button class="danger" type="button" data-wl-action="delete-tasting" data-id="${t.id}">Slett</button></div></div>`; };
  window.editTasting = function(id){ editTastingId=id; selectedTastingBottleId=''; page='tasting'; render(); setTimeout(()=>{const f=document.getElementById('tastingForm'); if(f)f.scrollIntoView({behavior:'smooth',block:'start'});},100); };
  window.deleteTasting = function(id){ if(!confirm('Slette denne smakingen?'))return; state.tastings=(state.tastings||[]).filter(t=>t.id!==id); save(); render(); };
  let ratingsBottleId='';
  window.renderOverview = function(){
    const totalValue=(state.bottles||[]).reduce((s,b)=>{try{return s+(Number(valueLeft(b.id))||0)}catch(e){return s}},0);
    const totalMl=(state.bottles||[]).reduce((s,b)=>{try{return s+(Number(bottleVolume(b.id))||0)}catch(e){return s}},0);
    const tastingCount=(state.tastings||[]).length;
    const ranked=(state.bases||[]).map(base=>({base,score:averageScoreForLibrary(base.id)})).filter(x=>Number(x.score)>0).sort((a,b)=>Number(b.score)-Number(a.score));
    shell(`<section class="hero"><h2>Oversikt / statistikk</h2><p class="sub">Rangering, score, verdi og historikk.</p></section><section class="grid statsGrid"><div class="card stat"><h2>${typeof money==='function'?money(totalValue):totalValue.toFixed(2)+' NOK'}</h2><p class="sub">verdi</p></div><div class="card stat"><h2>${Math.round(totalMl)} ml</h2><p class="sub">volum i beholdning</p></div><div class="card stat"><h2>${tastingCount}</h2><p class="sub">registrerte smakinger</p></div></section><section class="card ratingsMenuCard"><h2>Vurderinger</h2><p class="sub">Alle flaskescore med bilde, søk og detaljvisning.</p><button class="primary fullWidth" type="button" data-wl-action="open-ratings">Åpne vurderinger</button></section><section class="card"><h2>Flaskerangering</h2><div class="list">${ranked.length?ranked.map(x=>`<div class="item" data-wl-action="open-ratings">${img(x.base)}<div><div class="title">${esc(x.base.name||'')}</div><div class="meta"><b>${fmtPct(x.score)}</b></div></div></div>`).join(''):`<div class="sub">Ingen registreringer ennå.</div>`}</div></section>`,'overview');
  };
  window.renderRatings = function(){
    const q=(window.__wlRatingsSearch||'').trim().toLowerCase();
    let bottles=(state.bottles||[]).map(b=>{const base=bottleBase(b); const avg=wlBottleAvg(b.id); const count=(state.tastings||[]).filter(t=>t.bottleId===b.id).length; return {b,base,avg,count};}).filter(x=>x.base&&x.count>0);
    if(q)bottles=bottles.filter(x=>String(x.base.name||'').toLowerCase().includes(q)); bottles.sort((a,b)=>b.avg-a.avg);
    shell(`<section class="hero"><h2>Vurderinger</h2><p class="sub">Flaskescore sortert med høyeste score øverst.</p></section><section class="card"><label>Søk etter flaske</label><input id="wlRatingsSearch" placeholder="Søk etter flaske" value="${esc(window.__wlRatingsSearch||'')}"></section><section class="card"><div class="list">${bottles.length?bottles.map(x=>`<div class="item ratingsItem" data-wl-action="open-ratings-bottle" data-id="${x.b.id}">${img(x.base)}<div class="ratingsText"><div class="title">${esc(x.base.name||'')}</div><div class="meta"><b>${fmtPct(x.avg)}</b></div><div class="small">Antall smakinger: ${x.count}</div></div><div class="actions"><button class="ghost" type="button" data-wl-action="open-ratings-bottle" data-id="${x.b.id}">Åpne</button></div></div>`).join(''):`<div class="sub">Ingen vurderinger ennå.</div>`}</div></section>`,'ratings');
    const input=document.getElementById('wlRatingsSearch'); if(input)input.addEventListener('input',e=>{window.__wlRatingsSearch=e.target.value||''; renderRatings();});
  };
  window.renderRatingsBottle = function(){
    const b=getBottle(ratingsBottleId); const base=b?bottleBase(b):null; if(!b||!base){page='ratings';render();return;}
    const stats=wlBottleStats(b.id); const rows=(state.tastings||[]).filter(t=>t.bottleId===b.id).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
    shell(`<section class="hero">${img(base)}<h2>${esc(base.name||'')}</h2><p class="sub">Laveste: ${fmtPct(stats.min)} · Snitt: ${fmtPct(stats.avg)} · Høyeste: ${fmtPct(stats.max)}</p></section><section class="card"><h2>Smakinger</h2><p class="sub">Nyeste øverst.</p><div class="list">${rows.length?rows.map(tastingDetailItem).join(''):`<div class="sub">Ingen smakinger ennå.</div>`}</div></section>`,'ratingsBottle');
  };
  if(!window.__wlClick233){ window.__wlClick233=true; document.addEventListener('click',function(e){ const el=e.target.closest('[data-wl-action]'); if(!el)return; const a=el.dataset.wlAction; const id=el.dataset.id; e.preventDefault(); e.stopPropagation(); if(a==='open-ratings'){page='ratings';render();} else if(a==='open-ratings-bottle'){ratingsBottleId=id;page='ratingsBottle';render();} else if(a==='edit-tasting'){editTasting(id);} else if(a==='delete-tasting'){deleteTasting(id);} },true); }
  if(!window.__wlRender233){ window.__wlRender233=true; const old=window.render||render; window.render=function(){ if(page==='overview'){renderOverview();return;} if(page==='ratings'){renderRatings();return;} if(page==='ratingsBottle'){renderRatingsBottle();return;} old(); }; render=window.render; }
})();
