...
/* v1.77 clean settings rebuild - single source of truth */
window.WHISKYLOG_VERSION='1.77';

function wl77No(){ return settings && settings.language === 'no'; }
function wl77(en,no){ return wl77No() ? no : en; }

function wl77NearestSection(el){
  let n = el;
  while(n && n.parentElement && n.id !== 'settings'){
    const cls = String(n.className || '');
    if(n.tagName === 'SECTION' || cls.includes('card') || cls.includes('panel')){
      return n;
    }
    n = n.parentElement;
  }
...
function wl77RemoveLegacySettingsBlocks(){
  ...
  [...s.querySelectorAll('h1,h2,h3,p,button,div,section')].forEach(el => {
    if(!el || el.id === 'wl77SettingsExtras' || el.closest('#wl77SettingsExtras')) return;
    const txt = (el.textContent || '').trim();
    if(!txt) return;
    if(badNeedles.some(n => txt.includes(n))){
      const block = wl77NearestSection(el);
      if(block && block.id !== 'settings' && !block.closest('#wl77SettingsExtras')){
        block.remove();
      }
    }
  });
...
function wl77CreateRestorePoint(){
  const name = prompt(wl77('Restore point name','Navn på gjenopprettingspunkt'), wl77('Before cleanup','Før rydding'));
  if(name === null) return;
  ...
  points.unshift({
    id: uid(),
    name: String(name || wl77('Restore point','Gjenopprettingspunkt')).trim(),
  ...
  wl77BuildSettingsExtras();
  alert(wl77('Restore point created.','Gjenopprettingspunkt opprettet.'));
}

function wl77GetRestorePoints(){
  try{
    const p = JSON.parse(localStorage.getItem('whiskylog_restore_points') || '[]');
    return Array.isArray(p) ? p : [];
  }catch(e){ return []; }
}

function wl77RestorePoint(id){
  const p = wl77GetRestorePoints().find(x => x.id === id);
  if(!p) return;
  if(!confirm(wl77('Restore this point? Current data will be replaced.','Gjenopprette dette punktet? Gjeldende data blir erstattet.'))) return;
  ...
  save(); saveSettings(); render(); show('home');
}

function wl77DeleteRestorePoint(id){
  if(!confirm(wl77('Delete restore point permanently?','Slette gjenopprettingspunkt permanent?'))) return;
  localStorage.setItem('whiskylog_restore_points', JSON.stringify(wl77GetRestorePoints().filter(p => p.id !== id)));
  wl77BuildSettingsExtras();
}

function wl77RestoreListHtml(){
  const points = wl77GetRestorePoints();
  if(!points.length) return `<div class="sub">${wl77('No restore points yet.','Ingen gjenopprettingspunkter ennå.')}</div>`;
  return points.map(p => `
    <div class="item">
      <div>↩️</div>
      <div>
        ...
        <div class="wl77-actions">
          <button class="ghost" type="button" onclick="wl77RestorePoint('${p.id}')">${wl77('Restore','Gjenopprett')}</button>
          <button class="danger" type="button" onclick="wl77DeleteRestorePoint('${p.id}')">${wl77('Delete','Slett')}</button>
        </div>
      </div>
    </div>
  `).join('');
...
function wl77RestoreFromFile(file){
  const reader = new FileReader();
  reader.onload = () => {
    try{
      const data = JSON.parse(reader.result);
      ...
      if(!nextState || !Array.isArray(nextState.bases) || !Array.isArray(nextState.bottles)){
        throw new Error(wl77('Invalid backup file','Ugyldig backupfil'));
      }
      if(!confirm(wl77('Restore backup from file? Current data will be replaced.','Hente backup fra fil? Gjeldende data blir erstattet.'))) return;
      state = Object.assign({bases:[],bottles:[],tastings:[],comments:[],wishlist:[]}, nextState);
      if(data.settings){
        settings = Object.assign({ownerName:'Kenneth',currency:'NOK',language:'en',defaultTastingMl:20}, data.settings);
      }
      save(); saveSettings(); render(); show('home');
      alert(wl77('Backup restored.','Backup gjenopprettet.'));
    }catch(e){
      alert(wl77('Could not restore backup: ','Kunne ikke gjenopprette backup: ') + (e.message || ''));
    }
  };
  reader.readAsText(file);
}

function wl77BuildSettingsExtras(){
  const s = document.getElementById('settings');
  if(!s) return;
  wl77RemoveLegacySettingsBlocks();

  let old = document.getElementById('wl77SettingsExtras');
  if(old) old.remove();

  const wrap = document.createElement('div');
  wrap.id = 'wl77SettingsExtras';
  wrap.innerHTML = `
    <section class="card wl77-card">
      <h2>${wl77('Restore points','Gjenopprettingspunkter')}</h2>
      <p class="sub">${wl77(
      ...
      <button class="primary" type="button" id="wl77CreateRestore">${wl77('Create restore point','Opprett gjenopprettingspunkt')}</button>
      <div class="restore-list">${wl77RestoreListHtml()}</div>
    </section>
    <section class="card wl77-card">
      <button class="primary" type="button" id="wl77BackupFile">${wl77('Backup to file','Lagre backup til fil')}</button>
      <button class="ghost" type="button" id="wl77RestoreFile">${wl77('Restore from file','Hent backup fra fil')}</button>
      <input type="file" id="wl77RestoreInput" accept="application/json,.json" style="display:none">
      <p class="sub">${wl77(
  ...
  document.getElementById('wl77CreateRestore').onclick = wl77CreateRestorePoint;
  document.getElementById('wl77BackupFile').onclick = wl77BackupToFile;
  document.getElementById('wl77RestoreFile').onclick = () => {
    const inp = document.getElementById('wl77RestoreInput');
    inp.value = '';
    inp.click();
  };
  document.getElementById('wl77RestoreInput').onchange = e => {
    const file = e.target.files && e.target.files[0];
    if(file) wl77RestoreFromFile(file);
  };
}

function wl77FixGlobalLabels(){
  const mapNo = {saveAddNext:'Lagre og legg til neste', clearForm:'Tøm skjema', Edit:'Rediger', Delete:'Slett'};
  const mapEn = {saveAddNext:'Save & add next', clearForm:'Clear form', Rediger:'Edit', Slett:'Delete'};
  document.querySelectorAll('button').forEach(btn => {
    const t = (btn.textContent || '').trim();
    if(wl77No() && mapNo[t]) btn.textContent = mapNo[t];
    if(!wl77No() && mapEn[t]) btn.textContent = mapEn[t];
  });
...
const oldRender_v177 = render;
render = function(){
  oldRender_v177();
  setTimeout(()=>{ wl77BuildSettingsExtras(); wl77FixGlobalLabels(); }, 0);
  setTimeout(()=>{ wl77BuildSettingsExtras(); wl77FixGlobalLabels(); }, 300);
};

document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{wl77BuildSettingsExtras(); wl77FixGlobalLabels();},300));
document.addEventListener('click',()=>setTimeout(()=>{wl77BuildSettingsExtras(); wl77FixGlobalLabels();},150),true);
document.addEventListener('change',()=>setTimeout(()=>{wl77BuildSettingsExtras(); wl77FixGlobalLabels();},150),true);
setInterval(()=>{wl77BuildSettingsExtras(); wl77FixGlobalLabels();},2500);
