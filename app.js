...
const I18N = {
  ...
  no: {
    ...
    "No tastings.":"Ingen smakinger ennå.",
...
function bottleVolume(id){
  ...
  // If currentWeight is registered, weight is the source of truth.
  // Do not subtract tastings again; currentWeight already reflects real remaining contents.
  if(dec(b.currentWeight)>0){
    return Math.max(0, Math.round(volumeFromWeight(b)));
  }

  // Fallback only when no weight exists.
...
function openBottleDetail(id,returnView='home'){
  ...
  document.getElementById('bottleDetail').innerHTML=`
    ...
    <div class="card"><h3>Tastings</h3><div class="list">${state.tastings.filter(t=>t.bottleId===id).map(t=>`<div class="item"><div>📝</div><div><div class="title">${t.date} · ${tr('Average')} ${t.score||'—'}</div><div class="meta">${tr('Appearance')} ${t.appearance||'—'} · ${tr('Nose')} ${t.nose||'—'} · ${tr('Neat')} ${t.tasteNeat||'—'} · ${tr('Water')} ${t.tasteWater||'—'} · ${tr('Finish')} ${t.finish||'—'}</div><div class="sub">${esc(t.notes||'')}</div></div></div>`).join('')||('<div class="sub">'+tr('No tastings.')+'</div>')}</div></div>
  `;
  show('bottle-detail');
}
...
function deleteBottle(id){
  ...
  const ok=confirm('Delete bottle permanently?\n\nThis will permanently delete "'+name+'" and all tasting notes registered on this bottle.\n\nThis cannot be undone.');
  if(!ok)return;
